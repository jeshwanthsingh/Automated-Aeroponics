...
def lights():
    while True:
        try:
            print("turn on light for set time minute")
            data = requests.post("https://maker.ifttt.com/trigger/turn_on/with/key/8qnVquiUbPIPeKtd0jFfa")
            time.sleep(5)
            print("turn off light for set time minute")
            data = requests.post("https://maker.ifttt.com/trigger/turn_off/with/key/8qnVquiUbPIPeKtd0jFfa")
            time.sleep(5)
            if stop_threads:
                break
...
stop_threads = False
thread = threading.Thread(target=lights)
thread.start()
...
try:
    while True:
        ...
        print("Temp at shoot lvl:{0}, Temp above shoot lvl:{1}, ldr:{2}, Humi:{3}".format(rootTemp, shootTemp, ldr, humi))
        ...
        try:
            if ldr > 500:
                data = requests.post("https://maker.ifttt.com/trigger/High_brit/with/key/8qnVquiUbPIPeKtd0jFfa")
            else:
                data = requests.post("https://maker.ifttt.com/trigger/Low_brit/with/key/8qnVquiUbPIPeKtd0jFfa")    
        except requests.exceptions.RequestException:
            print("Network Error")
            pass
